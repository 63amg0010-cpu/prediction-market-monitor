"""Rebind the reviewed release root after the Vercel route repair."""
...
def _environment(name: str) -> str:
    ...
    if not value:
        raise RuntimeError("deployment_rebind_input_missing")
    return value


def _load_inputs() -> tuple[ReviewRoot, NoSpendReceipt, bytes, bytes]:
    root_hash = _environment("MIGRATION_DEPLOY_REBIND_REVIEW_ROOT_SHA256")
    no_spend_hash = _environment(
        "MIGRATION_DEPLOY_REBIND_NO_SPEND_RECEIPT_SHA256"
    )
    root = parse_body(
        _environment("MIGRATION_DEPLOY_REBIND_REVIEW_ROOT_B64"),
        root_hash,
        ReviewRoot,
    )
    no_spend = parse_body(
        _environment("MIGRATION_DEPLOY_REBIND_NO_SPEND_RECEIPT_B64"),
        no_spend_hash,
        NoSpendReceipt,
    )
    if (
        root.reviewed_sha
        != _environment("MIGRATION_DEPLOY_REBIND_EXPECTED_COMMIT_SHA")
        or root.approved_plan_sha256
        != _environment("MIGRATION_DEPLOY_REBIND_EXPECTED_PLAN_SHA256")
        or str(root.activation_nonce)
        != _environment("MIGRATION_DEPLOY_REBIND_ACTIVATION_NONCE")
        ...
        raise RuntimeError("deployment_rebind_binding_mismatch")
    return (
        root,
        no_spend,
...
def upgrade() -> None:
    ...
    if not bind.execute(
        ...
        raise RuntimeError("deployment_rebind_foundation_missing")
    prior_receipt = (
        bind.execute(
            text(
                """
    ...
    if not isinstance(prior_receipt, dict):
        raise RuntimeError("deployment_rebind_prior_root_invalid")
    if (
        ...
        raise RuntimeError("deployment_rebind_plan_or_identity_drift")
    ...
    receipt_body = {
        "schema_version": 1,
        "command": "release-correction-0010d",
        "reviewed_sha": root.reviewed_sha,
        ...
        "dispatch_nonce": _environment("MIGRATION_DEPLOY_REBIND_DISPATCH_NONCE"),
        "attempt": int(_environment("MIGRATION_DEPLOY_REBIND_ATTEMPT")),
        "state_before": "20260803_0010c",
    ...
    bind.execute(
        text(
            ...
            INSERT INTO release_receipt_chain (
                ...
                :receipt_sha, :receipt, 'release-correction-0010d',
                :reviewed_sha, :plan_sha, :approval_round_id,
...
def downgrade() -> None:
    """Remove only an unused 0010d root rebind."""
    if context.is_offline_mode():
        op.execute("SELECT 1")
        return
    bind = op.get_bind()
    correction_nonce = bind.execute(
        text(
            ...
            WHERE command = 'release-correction-0010d'
            """
        )
    ).scalar_one()
    ...
    if unexpected:
        raise RuntimeError("deployment_rebind_downgrade_dependency")
    bind.execute(
        text(
            """
