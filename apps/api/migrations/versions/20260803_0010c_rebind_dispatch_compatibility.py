"""Rebind the reviewed release root after the dispatch receipt compatibility repair."""
...
revision: str = "20260803_0010c"
down_revision: str | None = "20260803_0010b"
...
def _environment(name: str) -> str:
    ...
    if not value:
        raise RuntimeError("release_rebind_input_missing")
    return value


def _load_inputs() -> tuple[ReviewRoot, NoSpendReceipt, bytes, bytes]:
    root_hash = _environment("MIGRATION_DISPATCH_REBIND_REVIEW_ROOT_SHA256")
    no_spend_hash = _environment(
        "MIGRATION_DISPATCH_REBIND_NO_SPEND_RECEIPT_SHA256"
    )
    root = parse_body(
        _environment("MIGRATION_DISPATCH_REBIND_REVIEW_ROOT_B64"),
        root_hash,
        ReviewRoot,
    )
    no_spend = parse_body(
        _environment("MIGRATION_DISPATCH_REBIND_NO_SPEND_RECEIPT_B64"),
        no_spend_hash,
        NoSpendReceipt,
    )
    if (
        root.reviewed_sha
        != _environment("MIGRATION_DISPATCH_REBIND_EXPECTED_COMMIT_SHA")
        or root.approved_plan_sha256
        != _environment("MIGRATION_DISPATCH_REBIND_EXPECTED_PLAN_SHA256")
        or str(root.activation_nonce)
        != _environment("MIGRATION_DISPATCH_REBIND_ACTIVATION_NONCE")
        ...
        raise RuntimeError("release_rebind_binding_mismatch")
    return (
        root,
        no_spend,
...
def upgrade() -> None:
    ...
    if not bind.execute(
        ...
        raise RuntimeError("release_rebind_foundation_missing")
    prior_receipt = (
        bind.execute(
            text(
                """
    ...
    if not isinstance(prior_receipt, dict):
        raise RuntimeError("release_rebind_prior_root_invalid")
    if (
        ...
        raise RuntimeError("release_rebind_plan_or_identity_drift")
    ...
    receipt_body = {
        "schema_version": 1,
        "command": "release-correction-0010c",
        "reviewed_sha": root.reviewed_sha,
        ...
        "dispatch_nonce": _environment("MIGRATION_DISPATCH_REBIND_DISPATCH_NONCE"),
        "attempt": int(_environment("MIGRATION_DISPATCH_REBIND_ATTEMPT")),
        "state_before": "20260803_0010b",
    ...
    bind.execute(
        text(
            ...
            INSERT INTO release_receipt_chain (
                ...
                :receipt_sha, :receipt, 'release-correction-0010c',
                :reviewed_sha, :plan_sha, :approval_round_id,
...
def downgrade() -> None:
    """Remove only an unused 0010c root rebind."""
    if context.is_offline_mode():
        op.execute("SELECT 1")
        return
    bind = op.get_bind()
    correction_nonce = bind.execute(
        text(
            ...
            WHERE command = 'release-correction-0010c'
            """
        )
    ).scalar_one()
    ...
    if unexpected:
        raise RuntimeError("release_rebind_downgrade_dependency")
    bind.execute(
        text(
            """
            DELETE FROM release_receipt_chain
            WHERE activation_nonce = :nonce
            """
        ),
        {"nonce": correction_nonce},
