"""Rebind the release root after fixing activation reservation lookup."""
...
revision: str = "20260803_0010f"
down_revision: str | None = "20260803_0010e"
...
def _load_inputs() -> tuple[ReviewRoot, NoSpendReceipt, bytes, bytes]:
    root_hash = _environment("MIGRATION_ACTIVATION_REBIND_REVIEW_ROOT_SHA256")
    no_spend_hash = _environment(
        "MIGRATION_ACTIVATION_REBIND_NO_SPEND_RECEIPT_SHA256"
    )
    root = parse_body(
        _environment("MIGRATION_ACTIVATION_REBIND_REVIEW_ROOT_B64"),
        root_hash,
        ReviewRoot,
    )
    no_spend = parse_body(
        _environment("MIGRATION_ACTIVATION_REBIND_NO_SPEND_RECEIPT_B64"),
        no_spend_hash,
        NoSpendReceipt,
    )
    if (
        root.reviewed_sha
        != _environment("MIGRATION_ACTIVATION_REBIND_EXPECTED_COMMIT_SHA")
        or root.approved_plan_sha256
        != _environment("MIGRATION_ACTIVATION_REBIND_EXPECTED_PLAN_SHA256")
        or str(root.activation_nonce)
        != _environment("MIGRATION_ACTIVATION_REBIND_ACTIVATION_NONCE")
...
def upgrade() -> None:
    ...
    receipt_body = {
        "schema_version": 1,
        "command": "release-correction-0010f",
        "reviewed_sha": root.reviewed_sha,
        ...
        "dispatch_nonce": _environment(
            "MIGRATION_ACTIVATION_REBIND_DISPATCH_NONCE"
        ),
        "attempt": int(_environment("MIGRATION_ACTIVATION_REBIND_ATTEMPT")),
        "state_before": "20260803_0010e",
    ...
    bind.execute(
        text(
            ...
            INSERT INTO release_receipt_chain (
                ...
                :receipt_sha, :receipt, 'release-correction-0010f',
                :reviewed_sha, :plan_sha, :approval_round_id,
...
def downgrade() -> None:
    """Remove only an unused 0010f root rebind."""
    if context.is_offline_mode():
        op.execute("SELECT 1")
        return
    bind = op.get_bind()
    correction_nonce = bind.execute(
        text(
            ...
            WHERE command = 'release-correction-0010f'
            """
        )
    ).scalar_one()
