...
MIGRATION_INPUT_ENV_NAMES: Final = (
    ...
    "MIGRATION_OIDC_REBIND_REVIEW_ROOT_B64",
    "MIGRATION_OIDC_REBIND_REVIEW_ROOT_SHA256",
    "MIGRATION_OIDC_REBIND_NO_SPEND_RECEIPT_B64",
    "MIGRATION_OIDC_REBIND_NO_SPEND_RECEIPT_SHA256",
    "MIGRATION_OIDC_REBIND_EXPECTED_COMMIT_SHA",
    "MIGRATION_OIDC_REBIND_EXPECTED_PLAN_SHA256",
    "MIGRATION_OIDC_REBIND_ACTIVATION_NONCE",
    "MIGRATION_OIDC_REBIND_DISPATCH_NONCE",
    "MIGRATION_OIDC_REBIND_ATTEMPT",
    "MIGRATION_ZERO_PROVIDER_REBIND_REVIEW_ROOT_B64",
    "MIGRATION_ZERO_PROVIDER_REBIND_REVIEW_ROOT_SHA256",
    "MIGRATION_ZERO_PROVIDER_REBIND_NO_SPEND_RECEIPT_B64",
    "MIGRATION_ZERO_PROVIDER_REBIND_NO_SPEND_RECEIPT_SHA256",
    "MIGRATION_ZERO_PROVIDER_REBIND_EXPECTED_COMMIT_SHA",
    "MIGRATION_ZERO_PROVIDER_REBIND_EXPECTED_PLAN_SHA256",
    "MIGRATION_ZERO_PROVIDER_REBIND_ACTIVATION_NONCE",
    "MIGRATION_ZERO_PROVIDER_REBIND_DISPATCH_NONCE",
    "MIGRATION_ZERO_PROVIDER_REBIND_ATTEMPT",
    "GITHUB_RUN_ID",
    "RUNNER_TEMP",
)
...
def _bootstrap_environment(
    ...
    *,
    ...
    oidc_rebind_nonce = str(
        uuid5(NAMESPACE_URL, f"local-qa:{reviewed_sha}:oidc-rebind-activation")
    )
    oidc_rebind_root, oidc_rebind_no_spend = _release_pair(
        reviewed_sha,
        plan_sha,
        oidc_rebind_nonce,
        protected,
    ...
    canonical_rebind_nonce = str(
        uuid5(NAMESPACE_URL, f"local-qa:{reviewed_sha}:canonical-rebind")
    )
    canonical_rebind_root, canonical_rebind_no_spend = _release_pair(
        reviewed_sha,
        plan_sha,
        canonical_rebind_nonce,
        protected,
        "canonical-rebind",
    )
    zero_provider_rebind_nonce = str(
        uuid5(NAMESPACE_URL, f"local-qa:{reviewed_sha}:zero-provider-rebind")
    )
    zero_provider_rebind_root, zero_provider_rebind_no_spend = _release_pair(
        reviewed_sha,
        plan_sha,
        zero_provider_rebind_nonce,
        protected,
        "zero-provider-rebind",
    )
    return {
        ...
        "MIGRATION_OIDC_REBIND_REVIEW_ROOT_B64": base64.b64encode(
            oidc_rebind_root
        ).decode(),
        "MIGRATION_OIDC_REBIND_REVIEW_ROOT_SHA256": hashlib.sha256(
            oidc_rebind_root
        ).hexdigest(),
        "MIGRATION_OIDC_REBIND_NO_SPEND_RECEIPT_B64": base64.b64encode(
            oidc_rebind_no_spend
        ).decode(),
        "MIGRATION_OIDC_REBIND_NO_SPEND_RECEIPT_SHA256": hashlib.sha256(
            oidc_rebind_no_spend
        ).hexdigest(),
        "MIGRATION_OIDC_REBIND_EXPECTED_COMMIT_SHA": reviewed_sha,
        "MIGRATION_OIDC_REBIND_EXPECTED_PLAN_SHA256": plan_sha,
        "MIGRATION_OIDC_REBIND_ACTIVATION_NONCE": oidc_rebind_nonce,
        "MIGRATION_OIDC_REBIND_DISPATCH_NONCE": str(
        ...
        "MIGRATION_CANONICAL_REBIND_REVIEW_ROOT_B64": base64.b64encode(
            canonical_rebind_root
        ).decode(),
        "MIGRATION_CANONICAL_REBIND_REVIEW_ROOT_SHA256": hashlib.sha256(
            canonical_rebind_root
        ).hexdigest(),
        "MIGRATION_CANONICAL_REBIND_NO_SPEND_RECEIPT_B64": base64.b64encode(
            canonical_rebind_no_spend
        ).decode(),
        "MIGRATION_CANONICAL_REBIND_NO_SPEND_RECEIPT_SHA256": hashlib.sha256(
            canonical_rebind_no_spend
        ).hexdigest(),
        "MIGRATION_CANONICAL_REBIND_EXPECTED_COMMIT_SHA": reviewed_sha,
        "MIGRATION_CANONICAL_REBIND_EXPECTED_PLAN_SHA256": plan_sha,
        "MIGRATION_CANONICAL_REBIND_ACTIVATION_NONCE": canonical_rebind_nonce,
        "MIGRATION_CANONICAL_REBIND_DISPATCH_NONCE": str(
            uuid5(NAMESPACE_URL, f"local-qa:{reviewed_sha}:canonical-rebind-dispatch")
        ),
        "MIGRATION_CANONICAL_REBIND_ATTEMPT": "1",
        "MIGRATION_ZERO_PROVIDER_REBIND_REVIEW_ROOT_B64": base64.b64encode(
            zero_provider_rebind_root
        ).decode(),
        "MIGRATION_ZERO_PROVIDER_REBIND_REVIEW_ROOT_SHA256": hashlib.sha256(
            zero_provider_rebind_root
        ).hexdigest(),
        "MIGRATION_ZERO_PROVIDER_REBIND_NO_SPEND_RECEIPT_B64": base64.b64encode(
            zero_provider_rebind_no_spend
        ).decode(),
        "MIGRATION_ZERO_PROVIDER_REBIND_NO_SPEND_RECEIPT_SHA256": hashlib.sha256(
            zero_provider_rebind_no_spend
        ).hexdigest(),
        "MIGRATION_ZERO_PROVIDER_REBIND_EXPECTED_COMMIT_SHA": reviewed_sha,
        "MIGRATION_ZERO_PROVIDER_REBIND_EXPECTED_PLAN_SHA256": plan_sha,
        "MIGRATION_ZERO_PROVIDER_REBIND_ACTIVATION_NONCE": zero_provider_rebind_nonce,
        "MIGRATION_ZERO_PROVIDER_REBIND_DISPATCH_NONCE": str(
            uuid5(
                NAMESPACE_URL,
                f"local-qa:{reviewed_sha}:zero-provider-rebind-dispatch",
            )
        ),
        "MIGRATION_ZERO_PROVIDER_REBIND_ATTEMPT": "1",
        "GITHUB_RUN_ID": "1",
