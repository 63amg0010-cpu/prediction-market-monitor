...
CURRENT_PATTERN: Final = re.compile(
    r"^((?:2026072[67]_[0-9]{4})|20260803_0010[abcdefgh])(?: \(head\))?$"
)
...
def validate_dispatch(
    ...
    *,
    ...
    allowed = {
        ...
        ("upgrade", "20260803_0010h", "rebind-release-root"),
        ("upgrade", "20260727_0011", "migrate-production"),
        ("downgrade", "20260803_0010g", "rollback-manifold"),
        ("downgrade", "20260803_0010h", "rollback-manifold"),
    }
...
def validate_current(output: str, request: DispatchRequest) -> None:
    ...
    expected = (
        ...
        if request.revision == "20260803_0010e" and request.operation == "upgrade"
        else "20260803_0010e"
        ...
        if request.revision == "20260803_0010h" and request.operation == "upgrade"
        else "20260803_0010h"
        if request.revision == "20260727_0011"
        else "20260727_0011"
    )
