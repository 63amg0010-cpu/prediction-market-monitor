from io import StringIO
from pathlib import Path

from alembic import command
from alembic.config import Config
from alembic.script import ScriptDirectory

API_ROOT = Path(__file__).parents[2]


def test_0010a_is_the_inert_release_foundation_boundary() -> None:
    config = Config(str(API_ROOT / "alembic.ini"))
    script = ScriptDirectory.from_config(config)
    correction = script.get_revision("20260803_0010a")
    rebind = script.get_revision("20260803_0010b")
    dispatch_rebind = script.get_revision("20260803_0010c")
    deploy_rebind = script.get_revision("20260803_0010d")
    oidc_rebind = script.get_revision("20260803_0010e")
    activation_rebind = script.get_revision("20260803_0010f")
    canonical_rebind = script.get_revision("20260803_0010g")
    zero_provider_rebind = script.get_revision("20260803_0010h")
    head = script.get_revision("20260727_0011")

    assert correction is not None
    assert correction.down_revision == "20260727_0010"
    assert rebind is not None
    assert rebind.down_revision == "20260803_0010a"
    assert dispatch_rebind is not None
    assert dispatch_rebind.down_revision == "20260803_0010b"
    assert deploy_rebind is not None
    assert deploy_rebind.down_revision == "20260803_0010c"
    assert oidc_rebind is not None
    assert oidc_rebind.down_revision == "20260803_0010d"
    assert activation_rebind is not None
    assert activation_rebind.down_revision == "20260803_0010e"
    assert canonical_rebind is not None
    assert canonical_rebind.down_revision == "20260803_0010f"
    assert zero_provider_rebind is not None
    assert zero_provider_rebind.down_revision == "20260803_0010g"
    assert head is not None
    assert head.down_revision == "20260803_0010h"


def test_0010a_renders_generic_receipts_without_manifold_source() -> None:
    output = StringIO()
    config = Config(str(API_ROOT / "alembic.ini"), output_buffer=output)

    command.upgrade(config, "20260727_0010:20260803_0010a", sql=True)

    sql = output.getvalue()
    assert "CREATE TABLE release_receipt_chain" in sql
    assert "ADD COLUMN repository" in sql
    assert "ADD COLUMN claimed_run_attempt" in sql
    assert "ENABLE ROW LEVEL SECURITY" in sql
    assert "manifold-comments" not in sql
    assert "source_activation_attestations" not in sql


def test_0010a_offline_downgrade_removes_only_generic_foundation() -> None:
    output = StringIO()
    config = Config(str(API_ROOT / "alembic.ini"), output_buffer=output)

    command.downgrade(config, "20260803_0010a:20260727_0010", sql=True)

    sql = output.getvalue()
    assert "DROP TABLE release_receipt_chain" in sql
    assert "DROP COLUMN repository" in sql
    assert "DROP TABLE community_sources" not in sql


def test_0010b_is_an_inert_root_rebind_without_source_changes() -> None:
    output = StringIO()
    config = Config(str(API_ROOT / "alembic.ini"), output_buffer=output)

    command.upgrade(config, "20260803_0010a:20260803_0010b", sql=True)

    sql = output.getvalue()
    assert "SELECT 1" in sql
    assert "manifold-comments" not in sql
    assert "community_sources" not in sql


def test_0010c_is_an_inert_dispatch_compatibility_rebind() -> None:
    output = StringIO()
    config = Config(str(API_ROOT / "alembic.ini"), output_buffer=output)

    command.upgrade(config, "20260803_0010b:20260803_0010c", sql=True)

    sql = output.getvalue()
    assert "SELECT 1" in sql
    assert "manifold-comments" not in sql
    assert "community_sources" not in sql


def test_0010d_is_an_inert_deployment_route_rebind() -> None:
    output = StringIO()
    config = Config(str(API_ROOT / "alembic.ini"), output_buffer=output)

    command.upgrade(config, "20260803_0010c:20260803_0010d", sql=True)

    sql = output.getvalue()
    assert "SELECT 1" in sql
    assert "manifold-comments" not in sql
    assert "community_sources" not in sql


def test_0010e_is_an_inert_immutable_oidc_rebind() -> None:
    output = StringIO()
    config = Config(str(API_ROOT / "alembic.ini"), output_buffer=output)

    command.upgrade(config, "20260803_0010d:20260803_0010e", sql=True)

    sql = output.getvalue()
    assert "SELECT 1" in sql
    assert "manifold-comments" not in sql
    assert "community_sources" not in sql


def test_0010f_is_an_inert_activation_reservation_rebind() -> None:
    output = StringIO()
    config = Config(str(API_ROOT / "alembic.ini"), output_buffer=output)

    command.upgrade(config, "20260803_0010e:20260803_0010f", sql=True)

    sql = output.getvalue()
    assert "SELECT 1" in sql
    assert "manifold-comments" not in sql
    assert "community_sources" not in sql


def test_0010g_is_an_inert_canonical_receipt_rebind() -> None:
    output = StringIO()
    config = Config(str(API_ROOT / "alembic.ini"), output_buffer=output)

    command.upgrade(config, "20260803_0010f:20260803_0010g", sql=True)

    sql = output.getvalue()
    assert "SELECT 1" in sql
    assert "manifold-comments" not in sql
    assert "community_sources" not in sql


def test_0010h_is_an_inert_zero_provider_release_rebind() -> None:
    output = StringIO()
    config = Config(str(API_ROOT / "alembic.ini"), output_buffer=output)

    command.upgrade(config, "20260803_0010g:20260803_0010h", sql=True)

    sql = output.getvalue()
    assert "SELECT 1" in sql
    assert "manifold-comments" not in sql
    assert "community_sources" not in sql
