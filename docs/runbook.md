...
9. Keep `.github/workflows/verify.yml` at `*/15 * * * *`. Keep `.github/workflows/collect.yml` manual-only and dispatch it from the authenticated dashboard; do not add a schedule trigger. Historical cadence acceptance evidence remains append-only and must not be presented as evidence for the manual-only operating mode.
